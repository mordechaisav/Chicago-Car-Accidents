...
CSV_PATH = "data/Traffic_Crashes_-_Crashes.csv"
...
def count_accidents_by_area_route():
    ...
    print(area)
    count = count_accidents_by_area(car_accidents,area)
    return jsonify({"count": count,"area":area}), 200
...
def count_accidents_by_time_and_area_route():
    ...
    time_range = datetime.strptime(time_range, "%d-%m-%Y")
    count = count_accidents_by_time_and_area(car_accidents,area,time_period ,time_range)
    return jsonify({"count": count,"rang":str(time_range)}), 200


@accidents_bp.route('/accidents_grouped_by_cause', methods=['GET'])
def accidents_grouped_by_cause_route():
    area = request.args.get('area')
    accidents_grouped = get_accidents_grouped_by_cause(car_accidents,area)
...
def accidents_statistics_route():
    area = request.args.get('area')
    statistics = get_accidents_statistics(car_accidents,area)
